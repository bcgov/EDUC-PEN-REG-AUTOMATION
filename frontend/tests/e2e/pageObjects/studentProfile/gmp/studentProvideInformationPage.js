...
class studentProvideInformationPage {
    ...
    async uploadDocument(data) {
        ...
        await t.expect(this.uploadConfirmation.innerText).contains("File upload successful.", { timeout: 80000 })
    ...
    async verifyMaxFileSizeError(data) {
        await t.setFilesToUpload((this.selectFile), [data])
        log.info('file uploaded')
        await t.expect(this.canNotUploadLargeFileMessage.innerText).contains("File size should not be larger than 10 MB!", { timeout: 30000 })
